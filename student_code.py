...
def shortest_path(M, start, goal):
    ...
    # print(f"\nNodes coordinates are: {M.intersections}\n")
    # print(f"\nEdges are: {M.roads}\n")
    #Nodes are dictionary, where key is the node ID and the value is a coordinate List
    nodes = M.intersections
    #edges are a list of lists (adjacency list)
    edges = M.roads
    return A_star(nodes, edges, start, goal)

#==============================================================================
#==============================================================================
#https://stackoverflow.com/questions/5687882/what-are-some-good-methods-to-finding-a-heuristic-for-the-a-algorithm/5687914#5687914
def heuristic_distance(coord1, coord2):
    """
    The Euclidean distance between two points in Euclidean space is the length of a line segment between the two points.
    This insures that the heuristic distance <= the actual distance, as the straight line is the min distance between 2 points.
    # For testing purpose, use site :https://www.calculatorsoup.com/calculators/geometry-plane/distance-two-points.php
    The distance between two points is the length of the path connecting them. The shortest path distance is a straight line.
    In a 2 dimensional plane, the distance between points (X1, Y1) and (X2, Y2) is given by the Pythagorean theorem:
    𝑑=Square_root((𝑥2−𝑥1)^2+(𝑦2−𝑦1)^2)
...
def A_star(nodes, edges, start, goal):
    # Visited/expanded nodes are a set data structure because we only need this for checking the membership.
    expanded = set()
    # Frontier nodes with their paths are stored in a min heap data structure to speed up the search process
    current_paths = list()

    #step1: add the start node to the frontier min heap after calculating the total distance to the goal
    h_dist = heuristic_distance(nodes[start], nodes[goal])
    heapq.heappush(current_paths, (h_dist, [start]))

    #step2: loop over all possible paths towards the goal, until the goal reached with best dist.
    while len(current_paths)>0:
        min_dist, min_path_list = heapq.heappop(current_paths)
        #Expand the last node of the best possible path
        expanded.add(min_path_list[-1])
        #if it is the goal, then return the path
        if min_path_list[-1] == goal:
            return min_path_list
        #add the neighbors paths with dist = h(from neighbor to goal)+ actual path cost before neighbor
        for neighbor in edges[min_path_list[-1]]:
            if neighbor not in expanded:
                h_dist = heuristic_distance(nodes[neighbor], nodes[goal])
                path_cost = len(min_path_list)
                new_list = [node for node in min_path_list]
                new_list.append(neighbor)
                heapq.heappush(current_paths, ((path_cost+h_dist), new_list))
    return []
#==============================================================================
#==============================================================================
def test_A_start():
        #map1: 10 points
        nodes = {0: [0.7798606835438107, 0.6922727646627362], 1: [0.7647837074641568, 0.3252670836724646], 2: [0.7155217893995438, 0.20026498027300055], 3: [0.7076566826610747, 0.3278339270610988], 4: [0.8325506249953353, 0.02310946309985762], 5: [0.49016747075266875, 0.5464878695400415], 6: [0.8820353070895344, 0.6791919587749445], 7: [0.46247219371675075, 0.6258061621642713], 8: [0.11622158839385677, 0.11236327488812581], 9: [0.1285377678230034, 0.3285840695698353]}
        edges = [[7, 6, 5], [4, 3, 2], [4, 3, 1], [5, 4, 1, 2], [1, 2, 3], [7, 0, 3], [0], [0, 5], [9], [8]]
        answer = [5, 16, 37, 12, 34]
        start = 7
        goal = 4
        print(A_star(nodes, edges, start, goal))
